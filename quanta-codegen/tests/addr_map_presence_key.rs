...
#[test]
fn a_dirty_key_tail_cannot_slip_past_the_presence_guard() {
    let cc = compile(SRC);
    let owned = claim(&cc, 0x00, 0xAA, BTreeMap::new()).expect("the first claim succeeds");
    assert!(!owned.is_empty(), "the claim wrote an owner row");
    ...
    assert!(
        ...
        "an honest re-claim of an owned record must still be refused"
    );
}
