...
fn denies_over_truthy_non_one_operands_fires() {
    ...
    assert!(
        ...
        "a && b with a=2,b=1 is truthy, the deny must fire"
    );
    ...
    assert_eq!(
        out.get(&slot_key(0)),
        Some(&1),
        "the body ran when the deny did not fire"
    );
}
...
fn guard_over_truthy_non_one_operands_passes() {
    ...
    assert!(
        run(&cc, "act", &mem).is_err(),
        "both false fails the guard and reverts"
    );
}
...
fn not_of_a_truthy_non_one_operand_is_false() {
    ...
    assert!(
        run(&cc, "act", &mem).is_err(),
        "!2 is false, the guard must fail"
    );
...
fn a_nested_or_result_feeds_a_bitwise_and_correctly() {
    ...
    let mem = mem_with(
        &cc,
        "act",
        &[("order.a", 2), ("order.b", 0), ("order.c", 1)],
    );
    assert!(
        run(&cc, "act", &mem).is_err(),
        "(a||b) && c truthy, the deny must fire"
    );
    let mem = mem_with(
        &cc,
        "act",
        &[("order.a", 2), ("order.b", 0), ("order.c", 0)],
    );
